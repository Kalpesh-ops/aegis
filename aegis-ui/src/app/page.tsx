...
export default function LandingPage() {
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col">
      ...
      <div className="fixed top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-blue-400/10 dark:bg-blue-900/20 blur-[120px] pointer-events-none -z-10" />
      <div className="fixed bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-amber-400/10 dark:bg-amber-900/20 blur-[120px] pointer-events-none -z-10" />
      ...
      <main className="flex-grow pt-24">
        ...
        <section id="hero" className="max-w-7xl mx-auto px-6 py-20 flex flex-col items-center text-center">
          <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.1 } } }} className="space-y-8 max-w-4xl">
            <motion.div variants={fadeUp} className="inline-block px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-bold tracking-widest rounded-full border border-blue-200 dark:border-blue-800/50 uppercase">
            ...
            <motion.p variants={fadeUp} className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed font-light mx-auto max-w-2xl">
        ...
        {/* SYSTEM ENTRY (UPLOAD) */}
        <section id="upload" className="max-w-4xl mx-auto px-6 py-20 scroll-mt-24">
          <motion.div 
            initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 md:p-12 shadow-2xl"
          >
            <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl blur opacity-20 transition duration-1000 -z-10" />
            <div className="text-center mb-8">
              ...
              <h2 className="font-newsreader text-3xl font-semibold mb-2">Secure Ingestion Protocol</h2>
              <p className="text-slate-500 dark:text-slate-400">Upload the Master Tender Specification (PDF) to establish the deterministic baseline.</p>
            </div>
            <Dropzone onFileSelect={(file) => console.log('Process File', file)} />
          </motion.div>
        </section>

        {/* ARCHITECTURE OVERVIEW */}
        <section id="architecture" className="max-w-4xl mx-auto px-6 py-20 scroll-mt-24">
           <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="font-newsreader text-4xl font-semibold mb-12 text-center">System Architecture</h2>
        ...
        <section id="pillars" className="border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 py-24 scroll-mt-24">
          <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="p-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                <item.icon className="w-8 h-8 text-blue-600 dark:text-blue-400 mb-6" />
                <h3 className="font-newsreader text-xl font-semibold mb-3">{item.title}</h3>
