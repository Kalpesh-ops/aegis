...
export function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-500 ${isScrolled ? 'pt-6' : 'pt-0'}`}>
      <nav 
        className={`transition-all duration-500 ease-in-out flex items-center justify-between px-10 ${
          isScrolled 
            ? 'w-[95%] md:w-[90%] max-w-6xl py-4 bg-white/75 dark:bg-slate-950/75 backdrop-blur-2xl border border-slate-200/60 dark:border-slate-800/60 rounded-full shadow-[0_12px_40px_rgba(0,0,0,0.15)]' 
            : 'w-full max-w-7xl py-8 bg-white/0 dark:bg-slate-950/0 border-transparent rounded-none shadow-none'
        }`}
      >
        ...
        <div className="hidden md:flex items-center space-x-12 text-base font-medium">
          <Link href="#upload" className="text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors">
            System Entry
          </Link>
          <Link href="#architecture" className="text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors">
            Architecture
          </Link>
          <Link href="#pillars" className="text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors">
            Guardrails
          </Link>
        </div>
            ...
            <button 
              ...
              className={`p-2 rounded-full transition-all duration-300 ${
                isScrolled 
                  ...
                  : 'bg-white/10 hover:bg-white/20 dark:bg-slate-800/20 dark:hover:bg-slate-800/40 backdrop-blur-sm'
              }`}
